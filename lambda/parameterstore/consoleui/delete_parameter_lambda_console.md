# Deploy `delete_parameter.py` in AWS Lambda (Console UI)

Step-by-step guide to run `delete_parameter.py` as a Lambda function using the **AWS Console**.

## Prerequisites

1. Parameter exists.
2. **Permanent delete** — no recovery window.
...
| Setting | Value |
|--------|--------|
| **Function name** | `ssm-delete-parameter-demo` |
| **Runtime** | Python 3.12 |
| **Architecture** | x86_64 |
...
1. **Code** tab → paste full contents of `delete_parameter.py`
2. **Runtime settings** → **Handler:** `delete_parameter.lambda_handler`
3. Click **Deploy**
...
2. **Event name:** `DeleteParameterTest`
3. **Event JSON:**
...
{
  "statusCode": 200,
  "body": "{\"parameter_name\": \"/boto3-learning/app/config\", \"deleted\": true}"
}
```
...
| Error | Fix |
|-------|-----|
| `ParameterNotFound` | Parameter missing |
...
[ ] Handler: delete_parameter.lambda_handler
```
...
| File | Purpose |
|------|---------|
| `../delete_parameter.py` | Lambda handler source code |
