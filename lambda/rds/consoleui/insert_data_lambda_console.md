# Deploy `insert_data.py` in AWS Lambda (Console UI)

Step-by-step guide to run `insert_data.py` as a Lambda function using the **AWS Console**.
...
1. **Code** tab → paste full contents of `insert_data.py`
2. **Runtime settings** → **Handler:** `insert_data.lambda_handler`
3. Click **Deploy**
...
{
  "statusCode": 200,
  "body": "{\"message\": \"Row inserted successfully\", \"id\": 1}"
}
```
...
| Error | Fix |
|-------|-----|
| `Table missing` | Run create_table first |
...
| File | Purpose |
|------|---------|
| `../insert_data.py` | Lambda handler source code |
