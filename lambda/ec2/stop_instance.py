"""
Stop a running Amazon EC2 instance using boto3.
...
def stop_instance(instance_id: str) -> Dict[str, Any]:
    """Stop an EC2 instance (billing stops for compute; EBS storage continues)."""
    client = get_ec2_client()
    response = client.stop_instances(InstanceIds=[instance_id])
    state_change = response["StoppingInstances"][0]

    logger.info("Stopping instance %s (previous state: %s)", instance_id, state_change["PreviousState"]["Name"])
    return {
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "statusCode": 200,
            "body": json.dumps({"message": "Stop initiated successfully", **result}),
        }
