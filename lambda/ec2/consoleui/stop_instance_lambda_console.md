# Deploy `stop_instance.py` in AWS Lambda (Console UI)

Step-by-step guide to run `stop_instance.py` as a Lambda function using the **AWS Console**.
...
| Setting | Value |
|--------|--------|
| **Function name** | `lab-ec2-stop-instance` |
| **Runtime** | Python 3.12 |
| **Architecture** | x86_64 |
...
1. **Code** tab → paste full contents of `stop_instance.py`
2. **Runtime settings** → **Handler:** `stop_instance.lambda_handler`
3. Click **Deploy**
...
{
  "statusCode": 200,
  "body": "{\"message\": \"Stop initiated successfully\", \"current_state\": \"stopping\"}"
}
```
...
| Error | Fix |
|-------|-----|
| `IncorrectInstanceState` | Instance must be running |
...
| File | Purpose |
|------|---------|
| `../stop_instance.py` | Lambda handler source code |
